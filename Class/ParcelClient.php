    ...
    class ParcelClients extends Db {
        ...
        public function getClientOrderList($reg_id) {
            try {
                ...
                LEFT JOIN tbl_registration reg ON reg.reg_id=ck.reg_id LEFT JOIN tbl_menu m ON m.menu_id=ck.menu_id WHERE CK.reg_id=$reg_id");
                $stmt->execute();
        ...
        public function getParcelClients() {
            try {
                $stmt = $this->connect()->prepare("SELECT * FROM tbl_checkout ck 
                LEFT JOIN tbl_registration reg ON reg.reg_id=ck.reg_id LEFT JOIN tbl_menu m ON m.menu_id=ck.menu_id ORDER BY ck.order_date desc");
                $stmt->execute();
        ...
        public function viewParcelClients($regId, $checkoutId) {
            try {
                $stmt = $this->connect()->prepare("SELECT * FROM tbl_checkout ck LEFT JOIN tbl_registration reg ON reg.reg_id=ck.reg_id LEFT JOIN tbl_menu m ON m.menu_id=ck.menu_id LEFT JOIN tbl_category ct ON ct.cat_id=ck.cat_id WHERE ck.reg_id=:reg_id AND ck.checkout_id=:checkout_id ORDER BY ck.checkout_id");
                $stmt->execute([
                    'reg_id' => $regId,
